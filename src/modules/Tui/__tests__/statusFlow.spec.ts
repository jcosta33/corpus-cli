...
describe('run_status_flow', () => {
    it('renders the board and reports all clear for an empty workspace', async () => {
        const p = create_mock_prompter();
        expect(await run_status_flow(p, { workspaceDir: ws })).toBe(0);
        expect(p.calls.notes.some((n) => n.title === 'Board')).toBe(true);
        expect(p.calls.outros[0]).toBe('all clear');
    });

    it('flags attention items in the outro', async () => {
