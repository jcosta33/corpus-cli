...
describe('agents command (suspec agents emit, ADR-0098)', () => {
    it('emit --codex --from <dir> generates .codex/agents/*.toml, exit 0', async () => {
        ...
        expect(existsSync(join(cwd, '.codex', 'agents', 'suspec-reviewer.toml'))).toBe(true);
    });
    ...
    it('emit --json emits machine output with the written list', async () => {
        ...
        expect(parsed.written).toContain('suspec-reviewer.toml');
    });
    ...
    it('an unknown subcommand → exit 2, prints usage', async () => {
        ...
        expect(code).toBe(2);
        expect(err).toContain('suspec agents emit');
    });
    ...
    it('--force regenerates over an existing file', async () => {
        ...
        const path = join(cwd, '.codex', 'agents', 'suspec-reviewer.toml');
    ...
    it('with no --from, resolves ./.claude/agents when present (default source)', async () => {
        ...
        writeFileSync(
            join(localAgents, 'suspec-x.md'),
            '---\nname: suspec-x\ndescription: d\ntools: Read\n---\n\n# suspec-x\n\nb\n'
        );
        const { code } = await capture(() => run(['emit'], cwd));
        expect(code).toBe(0);
        expect(existsSync(join(cwd, '.codex', 'agents', 'suspec-x.toml'))).toBe(true);
    });

    it('with no --from and no local .claude/agents, falls back to the sibling catalog (errs in a bare temp cwd)', async () => {
        // a temp cwd has neither .claude/agents nor a sibling ../suspec-agents/agents → the fallback resolves
    ...
    it('suspec agents --help prints its usage, exit 0', async () => {
        const help = await capture(() => dispatch(['agents', '--help']));
        expect(help.code).toBe(0);
        expect(help.out).toContain('suspec agents emit');
    });
});
