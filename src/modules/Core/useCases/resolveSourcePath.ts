...
// puts shared artifacts at the WORKSPACE ROOT — `intake/x.md` (what `corpus pull` writes), `decisions/`,
...
// The workspace root for a spec file when only the file path is known (single-file `corpus check <file>`):
...
// overshoot to an ancestor monorepo (or a workspace nested inside another corpus repo) that also has one.
