...
spec_version: 0.1.0
status: draft
title: SOL lint — the five-layer linter over the IR (src/modules/Sol)
owners: []
imports: [sol-parser]
domain: architecture
created: 2026-06-08
...
# Spec: SOL lint — the five-layer linter

## Intent

The `Sol` module's linter runs the **five SOL lint layers** — `S` syntax, `P` prose, `M` semantic/cross-reference,
`V` verification, `O` orchestration/ownership — over the typed obligation IR produced by `sol-parser`,
...
- Not parsing — the IR and its parse-time diagnostics are `sol-parser` (IF-002, AC-005). This spec
  consumes that IR.
...
- Not proof **adequacy** judging (`SOL-V011`) — that needs a recorded trace/verdict and is a `verify`/`review`
  concern (the `verify`/`review` step). The `V` layer here checks the spec's verification *bindings*, not run proofs.
...
A **core module** at `src/modules/Sol` (ADR-0001), layered on the parser (`imports: sol-parser`); per `core-isolation` it cannot depend on `Commands`/`Terminal`. The five lint
...
ACCEPTS:
  - `ir: SwarmIR` (the typed obligation IR from `sol-parser`)
  - `options: { strict?: boolean }`
ERRORS:
  - ir-schema-invalid
OWNED BY Sol
VERIFY BY contract:cmdTest:src/modules/Sol/__tests__/lint.contract.spec.ts#returns_report_over_ir
...
OWNED BY Sol
VERIFY BY contract:cmdTest:src/modules/Sol/__tests__/report-schema.contract.spec.ts#report_matches_schema
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/layers.spec.ts#all_five_layers_run
DEPENDS ON IF-001
RISK high
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/verdict.spec.ts#verdict_aggregates_severities
DEPENDS ON IF-002
RISK high
...
VERIFY BY property:cmdTest:src/modules/Sol/__tests__/determinism.property.spec.ts#same_ir_same_deterministic_diagnostics
DEPENDS ON AC-001
RISK high
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/heuristic-prose.spec.ts#heuristic_p_is_advisory_and_marked
DEPENDS ON AC-002
RISK high
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/suggest-routing.spec.ts#diagnostics_carry_improve_op
DEPENDS ON AC-001
RISK low
...
VERIFY BY test:cmdTest:src/modules/Commands/__tests__/lint.spec.ts#nonzero_iff_blocking
DEPENDS ON AC-002
RISK medium
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/strict-mode.spec.ts#strict_promotes_demotable_to_blocking
DEPENDS ON AC-002
RISK medium
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/lexical-safety.spec.ts#hidden_chars_block
DEPENDS ON AC-001
RISK critical
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/readonly.spec.ts#source_and_ir_unchanged_after_lint
...
VERIFY BY static:cmdTest:src/modules/Sol/__tests__/lint.spec.ts#emitted_codes_in_catalogue
...
VERIFY BY test:cmdTest:src/modules/Sol/__tests__/report-schema.contract.spec.ts#heuristic_flag_present_on_p
...
VERIFY BY property:cmdTest:src/modules/Sol/__tests__/layer-code-agreement.property.spec.ts#code_layer_consistent
...
VERIFY BY property:cmdTest:src/modules/Sol/__tests__/verdict.property.spec.ts#verdict_is_pure_over_severities
...
| ID     | VERIFY BY                                                                                       |
| ------ | ----------------------------------------------------------------------------------------------- |
| IF-001 | contract:cmdTest:src/modules/Sol/__tests__/lint.contract.spec.ts#returns_report_over_ir            |
| IF-002 | contract:cmdTest:src/modules/Sol/__tests__/report-schema.contract.spec.ts#report_matches_schema    |
| AC-001 | test:cmdTest:src/modules/Sol/__tests__/layers.spec.ts#all_five_layers_run                          |
| AC-002 | test:cmdTest:src/modules/Sol/__tests__/verdict.spec.ts#verdict_aggregates_severities               |
| AC-003 | property:cmdTest:src/modules/Sol/__tests__/determinism.property.spec.ts#same_ir_same_deterministic_diagnostics |
| AC-004 | test:cmdTest:src/modules/Sol/__tests__/heuristic-prose.spec.ts#heuristic_p_is_advisory_and_marked  |
| AC-005 | test:cmdTest:src/modules/Sol/__tests__/suggest-routing.spec.ts#diagnostics_carry_improve_op        |
| AC-006 | test:cmdTest:src/modules/Commands/__tests__/lint.spec.ts#nonzero_iff_blocking                       |
| AC-007 | test:cmdTest:src/modules/Sol/__tests__/strict-mode.spec.ts#strict_promotes_demotable_to_blocking   |
| AC-008 | test:cmdTest:src/modules/Sol/__tests__/lexical-safety.spec.ts#hidden_chars_block                   |
| C-001  | test:cmdTest:src/modules/Sol/__tests__/readonly.spec.ts#source_and_ir_unchanged_after_lint         |
| C-002  | static:cmdTest:src/modules/Sol/__tests__/lint.spec.ts#emitted_codes_in_catalogue |
| C-003  | test:cmdTest:src/modules/Sol/__tests__/report-schema.contract.spec.ts#heuristic_flag_present_on_p   |
| I-001  | property:cmdTest:src/modules/Sol/__tests__/layer-code-agreement.property.spec.ts#code_layer_consistent |
| I-002  | property:cmdTest:src/modules/Sol/__tests__/verdict.property.spec.ts#verdict_is_pure_over_severities |
...
- Whether the cross-spec `SOL-M001` collision check needs a multi-document IR the parser does not yet build
  (Q-002) — it may force a small `sol-parser` extension or a separate workspace-level lint pass.
